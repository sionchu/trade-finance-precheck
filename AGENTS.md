...
- The current authorized gate is T5 — Treasury integration into the Single Deal Review Agent. Do not implement T6 from this gate.
...
- The Single Deal Review Agent is a one-shot, bounded read-only explanation layer. It must not perform authoritative financial calculations, mutate the Deal, execute finance, fetch external data, or retain conversation history. T5 may integrate already-computed Treasury context without changing these rules.

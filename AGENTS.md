...
- The current authorized gate is T6 — Final Product Completion / Presentation IA RE0 / Report / Submission Freeze. T6-A Company Liquidity Timeline, T6-B React Experience Shell, and T6-C Guided Decision / Agent UX Orchestration are implemented and frozen; the next authorized slice is T6-D Report / Submission / Final Product Freeze.
...
- The T6-B shell is an internal precompiled Streamlit Components v2 React bundle. React remains presentation/orchestration-only; `active_stage`, `review_goal`, and `response_action` are component state, Python formats authoritative values, and Motion respects reduced-motion preferences. T6-C adds the condition → choice → review → result → response loop without fake tool progress, retry, or another Agent/tool.

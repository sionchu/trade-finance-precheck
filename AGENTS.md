...
- The Single Deal Review Agent is a one-shot, bounded read-only explanation layer with exactly four local evidence tools. Its Treasury tool only serializes already-computed T1–T4 evidence. A successful run uses exactly two API requests, never fetches external data, and includes Treasury context in current-state freshness. Authoritative numbers remain deterministic UI output; the Agent must not calculate them, mutate the Deal, execute finance, or retain conversation history.
